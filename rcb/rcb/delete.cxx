		...
		if(Verity(std::filesystem::directory_entry(systemFilePath)).exists)
		{
			...
			if(aci::Stat(systemFilePath.string().c_str()).st_dev() == aci::Stat(g_singleton->getWorkingProgDir().string().c_str()).st_dev())
			{
				...
				catch(const std::filesystem::filesystem_error& e)
				{
					...
					if(!m_dOpt.dryRunOption)
						m_db.executeSQL(std::format("DELETE FROM {0} WHERE {1}=(SELECT MAX({1}) FROM {0});", g_kTableName, g_kSchemaID));
...
std::string Delete::incrementExecutionID()
{
	...
	std::string selectedExecution = m_db.selectData(std::format("SELECT {0} FROM {1} WHERE {0}=(SELECT max({0}) FROM {1}) AND {2}=(SELECT max({2}) FROM {1});", g_kSchemaExecution, g_kTableName, g_kSchemaID));
