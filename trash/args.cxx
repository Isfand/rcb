...
void Args::init()
{
	...
	{
		std::println("{0}: missing operand\ntry \'trash --help\' for more information", g_progName);
		return;
	}
	...
	for(auto arg : m_args)
	{
		...
		else if(std::string(arg) == "--version")
		{
			...
			std::println("\x1B[90m{}\033[0m", m_trash_ASCII_art);
			return;	
		}
		else
		{
			...
			std::println("Default Trash execute\nIncorrect use of command. Enter --help for usage");
			return;
		}
	}
